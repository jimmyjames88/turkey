...
export const tokenCommands = new Command('token')
    ...
    try {
      ...
      tokens.forEach(token => {
        const status = token.revokedAt ? '❌ Revoked' : 
                      new Date() > token.expiresAt ? '⏰ Expired' : '✅ Active';
      ...
      console.error('❌ Failed to list refresh tokens:', error instanceof Error ? error.message : error);
      process.exit(1);
    }
  });