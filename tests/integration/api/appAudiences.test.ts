  ...
  const tests: Array<() => Promise<TestResult>> = [
    ...
    async () => {
      ...
      if (result.success && result.data?.accessToken) {
        ...
        if (payload.aud === 'my_blog') {
          ...
          return { ...result, success: false, error: `Expected audience 'my_blog', got '${payload.aud}'` };
        }
      }
    ...
    async () => {
      ...
      if (photosLogin.success && financeLogin.success) {
        // Decode both tokens
        const photosPayload = JSON.parse(Buffer.from(photosLogin.data.accessToken.split('.')[1], 'base64').toString());
        const financePayload = JSON.parse(Buffer.from(financeLogin.data.accessToken.split('.')[1], 'base64').toString());
    ...
    async () => {
      ...
      if (result.status === 400 && !result.success) {
        return { ...result, success: true, data: { message: 'Invalid audience correctly rejected' } };
      } else {
    ...
    async () => {
      ...
      if (loginResult.success && loginResult.data?.refreshToken) {
        ...
        if (refreshResult.success && refreshResult.data?.accessToken) {
          const payload = JSON.parse(Buffer.from(refreshResult.data.accessToken.split('.')[1], 'base64').toString());
    ...
    async () => {
      ...
      if (result.success && result.data?.accessToken) {
        ...
        if (payload.aud === 'my_finance') {
          ...
          return { ...result, success: false, error: `Expected audience 'my_finance', got '${payload.aud}'` };
        }
      }
