...
async function testRateLimiting() {
  ...
  console.log(
    `${loginRateSuccess ? '✅' : '❌'} Login rate limiting - ${successfulLogins}/${loginAttempts} successful logins`
  )
  console.log('')
  ...
  console.log(
    `${bruteForceSuccess ? '✅' : '❌'} Brute force protection - Protection triggered: ${rateLimitHit || accountLocked}`
  )
  console.log('')
  ...
  if (loginForRefresh.status === 200 && loginForRefresh.data?.refreshToken) {
    ...
    console.log(
      `${refreshSuccess ? '✅' : '❌'} Refresh rate limiting - ${successfulRefreshes}/${refreshAttempts} successful refreshes`
    )
  } else {
    console.log(
      `❌ Could not get initial login for refresh test (status: ${loginForRefresh.status})`
    )
    console.log(
      `   This may be due to rate limiting from previous tests. User: ${refreshTestUser.email}`
    )
  }
  ...
  console.log(
    `${registrationSuccess ? '✅' : '❌'} Registration rate limiting - ${successfulRegistrations} successful, ${registrationAttempts} attempts, blocked: ${registrationBlocked}`
  )
  console.log('')
