...
async function quickAuthTest() {
  ...
  console.log(`   Registration: ${registerResult.status === 201 || registerResult.status === 409 ? 'OK' : 'FAILED'} (${registerResult.status})`);
  ...
  console.log(`   ${noTokenSuccess ? '✅' : '❌'} No token: ${noTokenResult.status} (Expected: 401)`);
  ...
  console.log(`   ${withTokenSuccess ? '✅' : '❌'} With token: ${withTokenResult.status} (Expected: 200)`);
