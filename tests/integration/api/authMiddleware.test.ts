...
async function testAuthenticationMiddleware() {
  ...
  console.log(
    `   Admin registration: ${adminRegSuccess ? 'OK' : 'FAILED'} (${adminRegResult.status})`
  )
  ...
  console.log(
    `${noAuthSuccess ? '✅' : '❌'} GET /v1/users/me (no token) - ${noAuthResult.status} (Expected: 401)`
  )
  ...
  console.log(
    `${invalidAuthSuccess ? '✅' : '❌'} GET /v1/users/me (invalid token) - ${invalidAuthResult.status} (Expected: 401)`
  )
  ...
  if (userToken) {
    ...
    console.log(
      `${userAuthSuccess ? '✅' : '❌'} GET /v1/users/me (user token) - ${userAuthResult.status} (Expected: 200)`
    )
  ...
  if (userToken) {
    ...
    console.log(
      `${userRoleSuccess ? '✅' : '❌'} GET /v1/users/profile (user token) - ${userRoleResult.status} (Expected: 200)`
    )
    console.log('')
  ...
  if (userToken) {
    ...
    console.log(
      `${userAdminSuccess ? '✅' : '❌'} GET /v1/users/admin-only (user token) - ${userAdminResult.status} (Expected: 403)`
    )
  ...
  if (adminToken) {
    ...
    console.log(
      `${adminAccessSuccess ? '✅' : '❌'} GET /v1/users/admin-only (admin token) - ${adminAccessResult.status} (Expected: 200)`
    )
  ...
  if (userToken) {
    ...
    console.log(
      `${tokenTestSuccess ? '✅' : '❌'} POST /v1/users/test-auth (user token) - ${tokenTestResult.status} (Expected: 200)`
    )
